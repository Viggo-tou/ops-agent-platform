...
7. **Conditional follow-up — cap=4500 中档**（codex Stage 12 critique D2，only-if 触发）：如果未来 FTS5/cards/hybrid 的 benchmark 数据**明确显示**有"宽但噪声多"的失败聚类（例：D-tier 某子集在 cap=6000 时 synthesis 把答案稀释，但在 cap=3000 时又截掉关键 trace），那时再试 cap=4500 全局或 per-tier。**触发条件不满足前不做**——v1+v2 经验证明，没数据驱动的 tier 加档纯粹是 churn。
