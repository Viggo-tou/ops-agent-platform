...
class CodeGenerator:
    ...
    def _try_android_job_default_address_recipe_codegen(
        ...
        *,
        ...
        if domain_id != "android_job_default_address" and not (
            {
        ...
        from app.services.android_job_default_address_recipe import (
        ...
        supported_names = {"jobpostingfragment.kt", "jobpostingflow.kt"}
        ...
        for target_path, prompt_content in targets:
            ...
            recipe_result = try_generate_android_job_default_address_recipe(
        ...
        logger.info(
            "android_job_default_address_recipe_applied files=%s latency_s=%.3f operations=%s",
            files_changed,
            latency_s,
            operations[:12],
        )
        ...
        return CodegenResult(
            diff="\n".join(diffs).strip() + "\n",
            summary=(
                "Generated Android job default-address patch via deterministic "
                "harness recipe"
            ),
            ...
            file_summaries=[
                {
                    "path": file_path,
                    "summary": "Applied Android job default-address harness recipe",
                }
                for file_path in files_changed
            ],
            attempt_history=[
                {
                    "provider": "harness:android_job_default_address_recipe",
                    "status": "succeeded",
                    "duration_s": latency_s,
                }
            ],
            provider_name="harness:android_job_default_address_recipe",
