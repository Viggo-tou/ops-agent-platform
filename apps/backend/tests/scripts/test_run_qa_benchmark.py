...
def test_extract_question_entities_finds_camelcase_classes() -> None:
    entities = bench.extract_question_entities(
        "How do CustomerJobListFragment, CustomerJobDetailsFragment, "
        "and CustomerJobListAdapter work together?"
    )
...
def test_extract_question_entities_finds_dotted_filenames() -> None:
    entities = bench.extract_question_entities(
        "Compare Login.js, ServiceAnalytics.js, Dashboard.js, firebase.js, and app.json."
    )
...
def test_extract_question_entities_finds_backticked_tokens() -> None:
    entities = bench.extract_question_entities(
        "Compare `src/pages/Login.js` with `CustomerKYC*` and CustomerKYCActivity."
    )

    assert entities == ["src/pages/Login.js", "CustomerKYC*", "CustomerKYCActivity"]


def test_summary_aggregates_multifile_coverage() -> None:
    artifact_root = BACKEND_ROOT / "tmp-pytest" / "summary_aggregates_multifile_coverage"
    records = [
        {
            "type": "question",
            "tier": "A",
            ...
            "expected_fact_in_card": True,
            "stage19_buckets": [],
            "multifile_mode_active": True,
            "coverage_rate": 0.5,
            "omitted_entities": ["Dashboard.js"],
        },
        {
            "type": "question",
            "tier": "B",
    ...
    summary = bench.build_summary(
        ...
        out_path=artifact_root / "qa-run.jsonl",
        backend_url="http://backend.test",
        actor_name="qa",
        requested_judge_mode="rule",
        judge=PassingJudge("rule"),
        started_at=bench.utc_now(),
        finished_at=bench.utc_now(),
        records=records,
        infrastructure_failed=False,
        running=False,
        question_timeout_seconds=bench.QUESTION_TIMEOUT_SECONDS,
        preflight_judge_status="pass",
        preflight_judge_error=None,
    )

    assert summary["multifile_mode_records"] == 2
    assert summary["multifile_mode_avg_coverage_rate"] == 0.75
    assert summary["total_omitted_entities"] == 1
